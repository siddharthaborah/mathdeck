...
function parsePrime(parser, lhs, order) {
  while (!parser.atEnd) {
...
function parseAssign(parser, lhs, until) {
  ...
  if (operator(lhs) === "Subscript" && symbol(operand(lhs, 1))) {
    ...
    if (!parser.getSymbolType(fn2).matches("indexed_collection")) {
      ...
      const subStr = (sub2 !== null && typeof sub2 === "string" ? sub2 : void 0) ?? (sub2 !== null && typeof sub2 === "number" ? String(sub2) : void 0);
...
function normalizeLocalAssign(expr) {
  ...
  if (!base) return expr;
  const sub = operand(lhs, 2);
  const subStr = (typeof sub === "string" ? sub : void 0) ?? (typeof sub === "number" ? String(sub) : void 0);
...
function normalizeAngle(degrees, mode) {
...
function degreesToDMS(totalDegrees) {
  ...
  let sec = Math.round(secDecimal * 1e3) / 1e3;
  let finalMin = min;
  let finalDeg = deg;
  if (sec >= 60) {
    sec = 0;
...
var DEFINITIONS_ARITHMETIC = [
  ...
  // BesselJ(n, x) represents J_n(x)
  {
    ...
    serialize: (serializer, expr) => {
      ...
      if (order !== null && x !== null) {
        return "\\operatorname{J}_{" + serializer.serialize(order) + "}" + serializer.wrapArguments(["BesselJ", x]);
      }
  ...
  },
  {
    ...
    serialize: (serializer, expr) => {
      ...
      const x = operand(expr, 2);
      if (order !== null && x !== null) {
        return "\\operatorname{Y}_{" + serializer.serialize(order) + "}" + serializer.wrapArguments(["BesselY", x]);
      }
  ...
  },
  {
    ...
    serialize: (serializer, expr) => {
      ...
      const x = operand(expr, 2);
      if (order !== null && x !== null) {
        return "\\operatorname{I}_{" + serializer.serialize(order) + "}" + serializer.wrapArguments(["BesselI", x]);
      }
  ...
  },
  {
    ...
    serialize: (serializer, expr) => {
      ...
      const x = operand(expr, 2);
      if (order !== null && x !== null) {
        return "\\operatorname{K}_{" + serializer.serialize(order) + "}" + serializer.wrapArguments(["BesselK", x]);
      }
...
function parseLog(command, parser) {
  ...
  if (parser.match("_")) sub = parser.parseGroup() ?? parser.parseToken();
  const args = parser.parseArguments("implicit");
  if (args === null && sub === null) return [command];
  if (args === null) return [command, sub];
  if (sub === null) return [command, ...args];
  if (sub === 10) return ["Log", args[0]];
  if (sub === 2) return ["Lb", ...args];
  return ["Log", args[0], sub];
}
...
function parseIntegral(command) {
  return (parser) => {
    ...
    while (!done) {
      ...
      let sup = null;
      let sub = null;
      while (!(sub !== null && sup !== null) && (parser.peek === "_" || parser.peek === "^")) {
        if (parser.match("_")) sub = parser.parseGroup() ?? parser.parseToken();
        else if (parser.match("^")) {
          sup = parser.parseGroup() ?? parser.parseToken();
        }
...
function unitToMathrm(expr) {
  ...
  if (op === "Power") {
    ...
    const expStr = typeof exp === "number" ? String(exp) : symbol(exp) ?? String(exp);
...
function prependIndexedEntry(index, trigger, entry) {
...
function makeParseHandler(entry, latexTrigger, idTrigger) {
  ...
  if (kind === "function") {
    ...
    const argMode = ("arguments" in entry ? entry.arguments : void 0) ?? "enclosure";
    if (fnName)
      return (parser, until) => {
    ...
    while (!parser.atEnd && !isCollection2 && !hasSubscriptEval) {
      ...
      if (hasBrace) {
        ...
        if (parser.options.strict === false && /^[0-9]$/.test(subToken)) {
          ...
          id2 += "_" + digits;
        } else if (/^[a-zA-Z0-9]$/.test(subToken) || /^\p{XIDS}$/u.test(subToken)) {
          parser.nextToken();
          id2 += "_" + subToken;
        } else {
          parser.index = underscoreIndex;
...
// src/compute-engine/latex-syntax/parse-number.ts
function parseOptionalSign(parser) {
  let isNegative = !!parser.matchAny(["-", "\u2212"]);
  while (parser.matchAny(["+", "\uFE62"]) || parser.skipSpace())
    if (parser.matchAny(["-", "\u2212"])) isNegative = !isNegative;
  return isNegative ? "-" : "+";
}
function parseDecimalDigits(parser, fmt, part = "whole") {
  const result = [];
  let done = false;
  while (!done) {
    ...
    const group = part === "whole" ? fmt.wholeDigitGroupSeparatorTokens : fmt.fractionalDigitGroupSeparatorTokens;
...
function parseSignedInteger(parser, fmt, part) {
  const start = parser.index;
...
function parseExponent(parser, fmt) {
  const start = parser.index;
  parser.skipVisualSpace();
...
function parseRepeatingDecimal(parser, fmt) {
  const start = parser.index;
  ...
  parser.index = start;
  if ((format === "auto" || format === "parentheses") && parser.matchAll([`\\left`, "("])) {
    repeatingDecimals = parseDecimalDigits(parser, fmt, "fraction");
  ...
  parser.index = start;
  if ((format === "auto" || format === "vinculum") && parser.matchAll([`\\overline`, "<{>"])) {
    repeatingDecimals = parseDecimalDigits(parser, fmt, "fraction");
  ...
  parser.index = start;
  if ((format === "auto" || format === "arc") && (parser.matchAll([`\\wideparen`, "<{>"]) || parser.matchAll([`\\overarc`, "<{>"]))) {
    repeatingDecimals = parseDecimalDigits(parser, fmt, "fraction");
    if (repeatingDecimals && parser.match("<}>"))
      return `(${repeatingDecimals})`;
    parser.index = start;
...
function parseNumber(parser, fmt) {
  const parseNumbersOption = parser.options.parseNumbers;
  ...
  if (startsWithdecimalSeparator || parser.match(".") || parser.matchAll(fmt.decimalSeparatorTokens)) {
  ...
  if (hasFractionalPart) {
    ...
    if (parser.match("\\ldots") || parser.matchAll(fmt.truncationMarkerTokens)) {
    }
  }
  ...
  if (!hasRepeatingPart && parser.options.parseNumbers === "rational") {
    ...
    if (!fractionalPart) {
      ...
      if (exponent) return ["Multiply", sign * whole2, ["Power", 10, exponent]];
      return numberExpression(sign * whole2);
    }
  ...
  return {
    num: (sign < 0 ? "-" : "") + wholePart + (hasFractionalPart ? "." + fractionalPart : "") + (exponent ? "e" + exponent : "")
  };
}
...
function describeTypeCallbackResult(value) {
  if (value === null) return "null";
...
var _Parser = class __Parser {
   ...
   */
  parseLatexNumber(isInteger = true) {
    let negative = false;
    let token = this.peek;
  ...
  parseRepeatingDecimal() {
    return parseRepeatingDecimal(this, this._numberFormatTokens);
  }
  parseNumber() {
    return parseNumber(this, this._numberFormatTokens);
  }
  ...
  tryParseBareSymbol() {
    ...
    const symbolName2 = __Parser.BARE_SYMBOL_MAP[name];
    if (!symbolName2) {
      this.index = start;
      return null;
    }
    ...
    while (this.peek === "_" || this.peek === "^") {
      if (this.match("_")) {
        ...
        if (this.match("_") || this.match("^"))
          ...
          sub ??= this.parseToken();
          if (sub === null && this.options.strict === false && this.peek === "(")
...
function parse(latex, dictionary, options) {
...
function serializeNumber(expr, options) {
  ...
  return sign + (result ?? serializeAutoNotationNumber(num, {
    ...options
  }));
}
  ...
  if (!m) {
    ...
    if (valString.indexOf(".") < 0) {
      if (valString.length === 1) {
        ...
        valString = sign + valString[0] + "." + valString.slice(1) + "e+" + (valString.length - 1).toString();
      }
    } else {
      ...
      if (!whole) {
        ...
        if (leadingZeros === fraction.length) {
          ...
          if (restDigits) {
            valString = sign + firstDigit + "." + restDigits + "e" + exponent2;
          } else {
            valString = sign + firstDigit + "e" + exponent2;
          }
        }
      } else {
        valString = sign + whole[0] + "." + whole.slice(1) + fraction + "e+" + (whole.length - 1).toString();
      }
    }
...
function toDecimalNumber(wholePart, fractionalPart, exp) {
  ...
  if (topLevel) {
    ...
    while (rest.length > 0) {
      ...
      if (ucMatch) {
        ...
        const [sub, rest2] = parseSymbolBody2(rest.substring(1), false, "none");
        subs.push(sub);
        rest = rest2;
      } else {
        break;
...
function defaultParseOptions(opts) {
  return {
    // NumberFormat fields
...
function defaultSerializeOptions(opts) {
  return {
    // NumberFormat fields
    imaginaryUnit: "\\imaginaryI",
    positiveInfinity: "\\infty",
...
var LatexSyntax = class {
  ...
  parse(latex, options) {
    ...
    return parse(latex, this.indexed, { ...defaults, ...options });
  }
  /**
  ...
  serialize(expr, options) {
    ...
    return serializeLatex(expr, this.indexed, { ...defaults, ...options });
  }
};
var _defaultSyntax = null;
function getDefaultSyntax() {
  _defaultSyntax ??= new LatexSyntax();
  return _defaultSyntax;
}
function parse2(latex) {
...
export {
  ...
  DEFINITIONS_PHYSICS as PHYSICS_DICTIONARY,
